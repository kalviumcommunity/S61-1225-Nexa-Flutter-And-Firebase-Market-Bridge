...
class _BuyerDashboardScreenState extends State<BuyerDashboardScreen>
  ...
  Future<void> _shareOrder(String productName, double price, String farmerName) async {
    try {
      await Share.share(
  ...
  List<Map<String, dynamic>> _filterAndSortOrders(List<QueryDocumentSnapshot> docs) {
    ...
    if (_searchQuery.isNotEmpty) {
      orders = orders.where((item) {
        final crop = (item['crop'] ?? item['productName'] ?? '').toString().toLowerCase();
        final farmer = (item['farmer'] ?? item['farmerName'] ?? '').toString().toLowerCase();
    ...
    if (_filterStatus != 'all') {
      orders = orders.where((item) => 
        (item['status'] ?? 'pending').toString().toLowerCase() == _filterStatus
      ).toList();
    }

    switch (_sortBy) {
      case 'price_low':
        orders.sort((a, b) => (a['totalAmount'] ?? a['price'] ?? 0)
            .compareTo(b['totalAmount'] ?? b['price'] ?? 0));
        break;
      case 'price_high':
        orders.sort((a, b) => (b['totalAmount'] ?? b['price'] ?? 0)
            .compareTo(a['totalAmount'] ?? a['price'] ?? 0));
        break;
      case 'oldest':
        orders.sort((a, b) {
          ...
          StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snap) {
              ...
              return Column(children: [
                Row(children: [
                  Expanded(child: _buildStatCard(Icons.pending_actions, 'Pending', '$pending')),
                  const SizedBox(width: 12),
                  Expanded(child: _buildStatCard(Icons.check_circle, 'Delivered', '$delivered')),
                ]),
                const SizedBox(height: 12),
                Row(children: [
                  Expanded(child: _buildStatCard(Icons.attach_money, 'Total Spent', '₹${spent.toInt()}')),
                  const SizedBox(width: 12),
                  Expanded(child: _buildStatCard(Icons.favorite, 'Favorites', '${favoriteListings.length}')),
                ]),
              ]);
            },
          ),
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                ...
                // Status filter
                FilterChip(
                  label: Text(_filterStatus == 'all' ? 'All Status' : _filterStatus),
  ...
  Widget _buildFilterOption(String label, String value) {
    return ListTile(
      title: Text(label),
      trailing: _filterStatus == value ? const Icon(Icons.check, color: Color(0xFF2196F3)) : null,
      onTap: () {
